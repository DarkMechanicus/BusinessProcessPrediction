...
parser.add_argument('--baseline_size', help='amount of models that will be trained to select the baseline from. Defaults to 500', type=int, default=500)
...
if sys_args.log_file:
    std_out = sys.stdout
    log_file = open('{}/baseline_training.log'.format(path), 'w')
    sys.stdout = log_file
...
for i in range(sys_args.baseline_size):
